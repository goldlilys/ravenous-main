...
function SearchBar({ searchYelp }) {
  ...
  const handleSortChange = (choice) => {
    ...
    // Trigger the search with the new sorting option when clicking on the links immediately
    searchYelp(keyword, location, choice);
  }
      ...
      <ul id="sort-options">
        {sortChoices.map((choice) => (
          <li
            key={choice.id}
            value={choice.id}
            onClick={() => handleSortChange(choice.id)} className={sort === choice.id ? "active" : ""}>{choice.choice}
          </li>
        ))}
      </ul>
      <hr />
